...
`int` 数组、`float` 数组以及 `string` 数组可以被搜索和排序，那么对于其他类型的数组呢，是不是我们必须得自己编程实现它们？
...
列表和树这样的数据结构，在它们的定义中使用了一种叫节点的递归结构体类型，节点包含一个某种类型的数据字段。
...
- [参考资料](https://github.com/Unknwon/the-way-to-go_ZH_CN/blob/master/eBook/11.9.md)
